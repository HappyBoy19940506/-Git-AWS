...
2. 为什么要用子网掩码呢？存在的意义是什么？
   ...
   如果划分成了一个一个大小的子网，就不需要地毯式搜索了。
   如果在同一个子网，直接通信。 如果不在同一个子网，需要交给Gateway去中介通信。
3. 有什么用处呢？用这种技术，具体怎么算的 不需要太多了解
   1. 分配子网掩码，可以帮助确定这个ip地址属于哪个网段，（哪个子网的）
     比如 255.255.255.0这个掩码，他可以让192.168.0.1 划归到192.168.0.0 到 192.168.0.x 这个网段
   2.利用子网掩码可以，帮你判断这个分配到的网段里面最多有几台机器，也就是1中 x的值是多少
     比如 255.255.255.0就是255台， 比如 255.255.255.240就是8台
